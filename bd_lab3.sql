...
select locations.location_id, street_address, city, state_province, country_name, department_name from locations natural join countries, departments;
  ...
  --2
SELECT first_name, last_name, department_id, department_name FROM employee JOIN departments USING (department_id);

  --3
SELECT first_name, last_name, job_id, department_name, employee_id
FROM employee JOIN departments USING (department_id) JOIN locations USING (location_id) WHERE city = 'London';

  --4
SELECT e.first_name, e.last_name, e.hire_date FROM employee e
JOIN employee j
ON (j.last_name ='Jones')
WHERE j.hire_date < e.hire_date;

  --5
SELECT department_name, COUNT(employee_id) FROM employee JOIN departments USING (department_id) GROUP BY department_name;

  --6
SELECT employee_id, job_title, CURRENT_DATE - employee.hire_date FROM employee JOIN jobs USING (job_id) WHERE department_id = 90;
...
WHERE location_id IN (SELECT location_id FROM locations WHERE country_id='US')));
  ...
  --4
SELECT first_name, last_name, job_title FROM employee JOIN jobs USING (job_id)
WHERE (employee_id IN (SELECT manager_id FROM employee));

  --5
SELECT first_name, last_name, salary FROM employee
WHERE salary = (SELECT min_salary FROM jobs WHERE employee.job_id = jobs.job_id);
